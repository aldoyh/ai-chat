...
function get_session_title(array $history, string $fallback): string {
    foreach ($history as $msg) {
        if (($msg['role'] ?? '') === 'user' && !empty($msg['content'])) {
            $title = trim(preg_replace('/\s+/', ' ', (string) $msg['content']));
            ...
            if (!$selectedModel) {
                ...
                usort($localModels, function($a, $b) {
                    ...
                    foreach ($keywords as $kw) {
                        if (stripos($a['name'], $kw) !== false) return -1;
                        if (stripos($b['name'], $kw) !== false) return 1;
                    }
                    return 0;
                });
...
if ($action === 'stream') {
    ...
    if (!$session_id || !$message) json_response(['error' => 'Invalid'], 400);
